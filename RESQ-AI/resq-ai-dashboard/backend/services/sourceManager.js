...
import { saveToMongoDB, loadFromMongoDB } from './mongoService.js';
...
export async function processSources() {
    ...
    await saveToMongoDB(latestCache);
}
...
export async function getLiveData() {
    ...
    if (mongoData && (mongoData.incidents.length > 0 || mongoData.reports.length > 0)) {
        return mongoData;
    }
    return latestCache;
}
