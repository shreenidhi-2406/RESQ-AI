        ...
        <>
            ...
            <div className="fixed inset-y-0 right-0 w-full md:w-[500px] bg-white shadow-2xl z-50 flex flex-col transform transition-transform duration-300">
                ...
                <div className="flex-1 overflow-y-auto px-6 py-6 space-y-7">
                    ...
                    {/* LOCATION SECTION */}
                    <div>
                        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Location & Coordinates</h3>
                        <div className="bg-slate-50 p-4 rounded-xl border border-slate-200/80 flex flex-col gap-2">
                            ...
                            {hasCoords && (
                                <div className="text-xs font-mono text-slate-700 pl-7 flex flex-wrap items-center gap-3 bg-amber-50/80 py-1.5 px-3 rounded border border-amber-200/60 mt-1">
                                    <span className="font-bold text-amber-900 uppercase tracking-wider text-[10px]">Real Coordinates:</span>
                                    <span>Latitude: <strong className="font-bold text-slate-900">{incident.latitude.toFixed(6)}</strong></span>
                                    <span>Longitude: <strong className="font-bold text-slate-900">{incident.longitude.toFixed(6)}</strong></span>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    {incident.source_url ? (
                        <a href={incident.source_url} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 w-full py-3.5 bg-slate-900 hover:bg-slate-800 text-white font-semibold rounded-xl transition-colors shadow-sm text-sm">
                            View Official Report <ArrowUpRight size={16} />
                        </a>
                    ) : (
                        <div className="flex items-center justify-center gap-2 w-full py-3.5 bg-slate-100 text-slate-400 font-semibold rounded-xl text-sm italic cursor-not-allowed">
